...
Next, load in a mouse GTF file (will be explained more in detail in \emph{Quick Start} section of this vignette):
...
For reference, these genomic coordinates were called using the SICER peak calling algorithm (Zang et al. 2009), and can be recreated by the user from the original sequencing files (deposited in the Gene Expression Omnibus), as specified in the instructions given in the publication.  Finally, type in the following command to annotate this peaks file with a GTF file (the R object \texttt{mouse}) whose genes have been extended 2000 bp upstream of their first exon and 500 bp downstream of their last exon:
