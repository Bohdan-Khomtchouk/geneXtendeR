...
\begin{figure}[H]
\centering
\includegraphics{figures/vignette_gene_annotate_example.png}
\caption{The figure shows three seperate genes that explemplify three of the four different scenarios that may arise when using the table.}
\end{figure}
...
\item The second gene has both a low mean distance as well as a relatively low SD, so "Zbtb16" is definitely a good gene to follow-up on and include the peaks near the gene in the follow-up.
\item And the third gene has 7 peaks that point to "Zfp423" that do not reside on the gene-body, with high mean and SD those 7 are probably unnecessary for use in a follow-up of that gene.
\item The default case where both the mean and sd are high. Some cleaning may be done but there is probably very little point in following-up on these peaks based on distance alone.
\end{enumerate}
