...
\texttt{geneXtendeR} also makes it possible to identify genes that are unique between two separate intervals, e.g., the 2300 and 2400 bp upstream extension levels\footnote{Note that after running the \texttt{hotspotPlot()} command earlier, we need to explicitly reset \texttt{peaksInput()} to specify that we are, once again, working with ``somepeaksfile.txt".}:
