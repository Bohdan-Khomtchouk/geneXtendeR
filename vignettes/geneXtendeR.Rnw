...
This automatically loads the \texttt{gtf} R package, which is a prerequisite data package containing the gene transfer format files of commonly studied model organisms in ChIP-seq analyses.  Refer to the \texttt{gtf} R package for more details.
...
First, create a series of \texttt{geneXtendeR} files at some user-specified interval:
...
This command generates 21 individual whole-genome files: 0, 500, 1000, ..., and 10000 bp upstream extension files for the rat (\emph{Rattus norvegicus}) genome, each having an automatic 500 bp downstream extension.  See \texttt{species()} for a list of available genomes from the \texttt{gtf} R package.
...
This command properly formats the user's peak file in preparation for subsequent analyses.  For sake of reproducibility, ``myfilename.txt" is provided in the \texttt{/vignettes} directory of the geneXtendeR package to allow for an interactive session with the commands introduced in the rest of this vignette.
